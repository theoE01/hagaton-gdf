...
class SubmissionAnalise(db.Model):
    __tablename__ = "submission_analises"

    id = db.Column(db.Integer, primary_key=True)

    # 1 análise por submission (padrão)
    submission_id = db.Column(db.Integer, db.ForeignKey("submission.id"), nullable=False, unique=True)

    # redundância útil p/ consultas rápidas
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    protocolo = db.Column(db.String(36), nullable=False, index=True)

    categoria = db.Column(db.String(60), nullable=False, default="não_classificado")
    prioridade = db.Column(db.String(20), nullable=False, default="baixa")  # baixa|media|alta|critica
    tags_json = db.Column(db.Text, nullable=False, default="[]")            # lista em JSON
    resumo_curto = db.Column(db.String(280), nullable=False, default="")

    # opcional: guardar saída bruta/explicações
    resultado_json = db.Column(db.Text, nullable=True)  # JSON completo retornado pelo modelo
    modelo = db.Column(db.String(80), nullable=True)

    criado_em = db.Column(db.DateTime(timezone=True), server_default=func.now())
    atualizado_em = db.Column(db.DateTime(timezone=True), onupdate=func.now())

    submission = db.relationship("Submission", backref=db.backref("analise", uselist=False))
